...
import fay_booter
from ai_module import baidu_emotion, opencv
...
from ai_module import nlp_dify_api
...
def send_for_answer(msg):
        user = None
        if fay_booter.feiFei.user is not None:
            user = fay_booter.feiFei.user
            print('当前用户：' + user)
...
class FeiFei:
    ...
    def signin(self, styleType):
        contentdb = content_db.new_instance()
        # 当前时间
        timestamp = datetime.now().strftime('%Y-%m-%d %H:%M:%S')

        # 服务器的URL
        url = 'http://192.168.3.63:8766/signin'

        # 发送POST请求
        response = requests.post(url, json={'time': timestamp, 'user': self.user})

        # 检查响应状态码
        if response.status_code == 200:
            text = "签到成功"
        else:
            text = "签到失败，状态码：" + str(response.status_code)
                ...
                if len(self.interactive) > 0:
                    ...
                    if index == 1:
                        ...
                        if not config_util.config["interact"]["playSound"]: # 非展板播放
                            ...
                            wsa_server.get_instance().add_cmd(content)
                        # #fay eyes
                        # fay_eyes = face_recognize.new_instance()
                        # if fay_eyes.get_status():#人脸识别正在运行
                        #     self.user = fay_eyes.get_user()
    ...
    def recognize_message(self, styleType, user):
        self.user = user
        contentdb = content_db.new_instance()
        if user is None:
            text = "识别失败"
    ...
    def __send_or_play_audio(self, file_url, say_type):
        try:
            ...
            if config_util.config["interact"]["playSound"]: # 展板播放
                ...
                # print("111111111111111111111111111111111111" + content)
