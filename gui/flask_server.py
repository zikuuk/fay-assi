...
def control_eyes():
    ...
    eyes = face_recognize.new_instance()
    eyes.rtsp_url = config_data['interact']['rtspUrl']
...
@__app.route('/face/hello', methods=['get'])
def hello():
    time1 = datetime.now()
    print('in:' + str(time1))
    user = request.args.get('user')
    MyThread(target=fay_booter.feiFei.recognize_message, args=['interact', user]).start()
    return "success"

@__app.route('/face/read', methods=['get'])
def read():
    text = request.args.get('text')
    MyThread(target=fay_booter.feiFei.read, args=['interact', text]).start()
    return "success"
